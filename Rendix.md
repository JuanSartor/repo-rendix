...
1. **✅ Landing con lista de espera** (hecha 2026-08-10, `landing/index.html`): la frase, un mock del comparativo nominal vs real (números ilustrativos, no el screenshot real — la cartera de prueba tiene retornos absurdos de +1000% que le sacarían credibilidad), tabla comparativa vs. competencia, y el form de email contra `POST /api/waitlist`. Es HTML estático standalone, sin build ni dependencia del resto del frontend, para poder deployar ya (Vercel, Netlify, o cualquier hosting estático) sin esperar a nada más. Antes de deployar: cambiar `API_URL` en el `<script>` a la URL real del backend, y agregar el dominio de la landing a `ALLOWED_ORIGINS` en el backend.
